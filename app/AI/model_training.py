...
class Training:
    ...
    def __init__(self, filename, model_id, user_id):
        ...
        try:
            ...
            if not os.path.exists('training_weights'):
                os.mkdir('training_weights')

            today = datetime.now()
            h = today.hour
            m = today.minute

            sb_folder = today.strftime('%Y_%m_%d_') + str(h) + '_' + str(m)

            self.sub_folder = sb_folder
    ...
    def filter_data(self):
        ...
        try:
            for lead in range(0, len(self.df)):
                leads = self.df['Leads'][lead]
                leads = leads.lower()
                leads = leads.encode('ascii', errors='ignore')
                leads = str(leads)
                lead_tokenize = leads.split(" ")
                for word in lead_tokenize:
                    word = word.replace('\\', '/')
                    if ('job' in word) or ('hiring' in word) or ('hire' in word):
                        self.filtered_list.append(word)
                    elif ('http' in word) or ('@' in word) or ('#' in word):
                        continue
                    else:
                        self.filtered_list.append(word)
                leads = ' '.join(self.filtered_list)
                if len(leads) > 1:
                    if leads[0] == 'b' and (leads[1] == "'" or leads[1] == '"'):
                        leads = leads[2:]
                self.df['Leads'][lead] = leads
                self.filtered_list = []
            logging.info('Total wheat and chaff {}'.format(self.df.wheat_or_chaff.value_counts()))
        except Exception as e:
            logging.error('ERROR {}'.format(e))
    ...
    def cleaning(self):
        ...
        try:
            ...
            self.df['Leads'] = self.df.apply(lambda x: Training.clean_text(self, x.Leads), axis=1)
            self.df['Leads'] = self.df['Leads'].str.replace('\d+', '')
            logging.info('X:input feature shape  {}'.format(self.df["Leads"].values.shape))
            logging.info('y:output feature shape {}'.format(self.df.drop(['Leads'], axis=1).values.shape))
        except Exception as e:
            logging.error('ERROR {}'.format(e))
    ...
    def word_embedding(self):
        ...
        try:
            ...
            tokenizer.fit_on_texts(self.df['Leads'].values)
            ...
            with open('app/AI/weights/custom/'+self.user_id+'/'+self.model_id+'/'+tokenizer_model, 'wb') as handle:
                pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)
            self.X = tokenizer.texts_to_sequences(self.df['Leads'].values)
            self.X = pad_sequences(self.X, maxlen=self.max_sequence_length)
            logging.info('Shape of data tensor {}'.format(self.X.shape))
            self.y = pd.get_dummies(self.df['wheat_or_chaff']).values
            logging.info('Shape of label tensor {}'.format(self.y.shape))
            self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.X, self.y,
                                                                                    test_size=float(splitting_ratio),
                                                                                    random_state=42)
        except Exception as e:
            logging.error('ERROR {}'.format(e))
    ...
    def model_architecture(self):
        ...
        try:
            ...
            self.model.add(Dense(int(num_of_classes), activation='sigmoid'))
...
if __name__ == "__main__":
    ...
    train_obj = Training(input_file_name)
